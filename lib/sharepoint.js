...
async function listFolder(driveId, folderPath, token) {
    const files = new Map(); // filename -> { id, createdDateTime, lastModifiedDateTime, createdBy, size }
    let apiCalls = 0;
    let url = `https://graph.microsoft.com/v1.0/drives/${encodeURIComponent(driveId)}/root:/${encodeURI(folderPath)}:/children?$select=name,id,createdDateTime,lastModifiedDateTime,createdBy,size&$top=200`;
    ...
    for (let i = 0; i < scopes.length; i += concurrency) {
        ...
        await Promise.all(
            batch.map(async ([scope, entries]) => {
                const { files: existingFiles, apiCalls } = await listFolder(driveId, scope, token);
                ...
                for (const { docUrl, filename } of entries) {
                    if (!folderExists) {
                        ...
                        const fileInfo = existingFiles.get(filename);
                        const exists = !!fileInfo;
                        results[docUrl] = exists;
                        if (exists) {
                            metadata[docUrl] = fileInfo;
                            found++;
                        } else {
                            missing++;
                        }
...
export async function getThumbnails(itemId) {
    const token = await getAccessToken();
    const env = await getEnv();
    const driveId = env.SP_DRIVE_ID;

    const resp = await fetch(
        `https://graph.microsoft.com/v1.0/drives/${encodeURIComponent(driveId)}/items/${encodeURIComponent(itemId)}/thumbnails`,
        {
            headers: { Authorization: `Bearer ${token}` },
        }
    );
    ...
    if (data.value && data.value.length > 0) {
        ...
        return {
            small: set.small?.url || null,
            medium: set.medium?.url || null,
            large: set.large?.url || null,
        };
    }
    return null;
}
