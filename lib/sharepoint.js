    ...
    for (let i = 0; i < scopes.length; i += concurrency) {
        ...
        await Promise.all(
            batch.map(async ([scope, entries]) => {
                const existingFiles = await searchScope(driveId, scope, token);

                for (const { docUrl, filename } of entries) {
                    if (existingFiles === null) {
                        // Folder doesn't exist or search failed — all files missing
                        results[docUrl] = false;
                    } else {
                        results[docUrl] = existingFiles.has(filename);
                    }
                }
            })
        );
